...
<html>
  ...
  <html:form action='/index' method="post">
	...
	<c:if test="${loop.count>1}"> 
    <c:set var="out" value='&gt;&gt;<a href="${base}/index.do?simpleForumList=true&parent.id=${menu.id}&id=${param.id}">${menu.name}</a>${out}'/>
	</c:if>
	</c:forEach>
	<div style="background-color:#9ECDE4;margin:0;padding:0">
	<c:if test="${out!=null && out!=''}"><a href="${base}/index.do?simpleForumList=true&id=${param.id}">顶级版块</a></c:if>
	...
	<!--#######################################Loop开始#########################################-->
	 <c:forEach items="${pageView.records}" var="entity" varStatus="vs">
      ...
      <td>
          <c:if test="${fn:length(entity.children)>0}"><a href="<html:rewrite action='/index?simpleForumList=true&parent.id=${entity.id}&id=${param.id}'/>">${entity.name}</a></c:if>
